...
training3D_matrix = joblib.load('../data_processing/matrix3D_ISEAR_fasttext')
#Load Training Matrix
...
ff = Dense(100)(ff)
ff = Dropout(0.3) (ff)
ff =Dense(7, activation='softmax') (ff)
...
filepath="weights.{epoch:02d}-{val_loss:.2f}_fasttext.hdf5"
checkpoint = ModelCheckpoint(filepath, monitor='val_acc', verbose=1, save_best_only=True, mode='max')

callbacks_list = [
checkpoint,
    MyCallBack(X_test,Y_test,verbose=1)
]



m.compile ( loss='categorical_crossentropy' , optimizer='adam' , metrics=['accuracy'] )
history = m.fit(X_train,Y_train,64,100,
