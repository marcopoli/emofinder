...
training3D_matrix = joblib.load('../data_processing/matrix2D_ISEAR_fasttext_flatten')
#Load Training Matrix
#test3D_matrix = joblib.load('../data_processing/matrix3D_google_general_all_dev')

#Load Dataset
dataset = pd.read_csv( '../data_processing/dataConverted.csv',delimiter=',')
training_classes = dataset['emotion']
...
gnb = GaussianNB()# RandomForestClassifier(n_estimators=500)# SVC(kernel='rbf',gamma=0.2, C=1.5) #GaussianNB()
...
y_TT = le.fit_transform(Y_test.tolist())
...
re = recall_score ( y_pred , Y_test , average='micro')
print ( " Rec:" , re )
